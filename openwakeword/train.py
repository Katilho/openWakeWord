...
class Model(nn.Module):
    def __init__(
        ...
        if model_type == "dnn":
            ...
            class Net(nn.Module):
                def __init__(self, input_shape, layer_dim, n_blocks=1, n_classes=1):
                    ...
                    self.blocks = nn.ModuleList(
                        [FCNBlock(layer_dim) for i in range(n_blocks)]
                    )
            ...
            self.model = Net(
                input_shape, layer_dim, n_blocks=n_blocks, n_classes=n_classes
            )
        elif model_type == "rnn":
        ...
        if n_classes == 1:
            ...
            def multiclass_fp(p, y, threshold=0.5):
                ...
                neg_ndcs = y == 0
                fp = (
                    probs[neg_ndcs].argmax(axis=1)
                    != 0 & (probs[neg_ndcs].max(axis=1)[0] > threshold)
                ).sum()
                return fp
            ...
            def positive_class_recall(p, y, negative_class_label=0, threshold=0.5):
                ...
                pos_ndcs = y != 0
                rcll = (
                    probs[pos_ndcs].argmax(axis=1)
                    > 0 & (probs[pos_ndcs].max(axis=1)[0] >= threshold)
                ).sum() / pos_ndcs.sum()
                return rcll
            ...
            def positive_class_accuracy(p, y, negative_class_label=0):
                ...
                acc = (
                    probs[pos_preds].argmax(axis=1) == y[pos_preds]
                ).sum() / pos_preds.sum()
                return acc
    ...
    def summary(self):
        return torchinfo.summary(
            self.model, input_size=(1,) + self.input_shape, device="cpu"
        )
    ...
    def _select_best_model(
        ...
        for batch in false_positive_validate_data:
            ...
            for mdl_ndx, model in tqdm(
                enumerate(self.best_models),
                total=len(self.best_models),
                desc="Find best checkpoints by false positive rate",
            ):
                with torch.no_grad():
                    val_ps = model(x_val)
                    false_positive_rates[mdl_ndx] = (
                        false_positive_rates[mdl_ndx]
                        + self.fp(val_ps, y_val[..., None]).detach().cpu().numpy()
                    )
        false_positive_rates = [fp / val_set_hrs for fp in false_positive_rates]

        candidate_model_ndx = [
            ndx for ndx, fp in enumerate(false_positive_rates) if fp <= max_fp_per_hour
        ]
        candidate_model_recall = [
            self.best_model_scores[ndx]["val_recall"] for ndx in candidate_model_ndx
        ]
        if max(candidate_model_recall) <= min_recall:
            logging.warning(f"No models with recall >= {min_recall} found!")
            return None
        else:
            best_model = self.best_models[
                candidate_model_ndx[np.argmax(candidate_model_recall)]
            ]
            best_model_training_step = self.best_model_scores[
                candidate_model_ndx[np.argmax(candidate_model_recall)]
            ]["training_step_ndx"]
            logging.info(
                f"Best model from training step {best_model_training_step} out of {len(candidate_model_ndx)}"
    ...
    def auto_train(
        ...
        if self.best_val_fp > target_fp_per_hour:
            ...
            logging.info(
                "Increasing weight on negative examples to reduce false positives..."
            )
        ...
        # Merge best models
        logging.info(
            "Merging checkpoints above the 90th percentile into single model..."
        )
        ...
        with torch.no_grad():
            ...
            combined_model_recall = (
                self.recall(val_ps, y[..., None]).detach().cpu().numpy()
            )
            combined_model_accuracy = (
                self.accuracy(val_ps, y[..., None].to(torch.int64))
                .detach()
                .cpu()
                .numpy()
            )
            ...
            combined_model_fp_per_hr = (
                (combined_model_fp / val_set_hrs).detach().cpu().numpy()
            )
    ...
    def train_model(
        ...
        for step_ndx, data in tqdm(enumerate(X, 0), total=max_steps, desc="Training"):
            ...
            neg_high_loss = predictions[
                (y == 0) & (predictions.squeeze() >= 0.001)
            ]  # thresholds were chosen arbitrarily but work well
            ...
            if predictions.shape[0] != 0:
                ...
                loss = self.loss(
                    predictions, y_ if self.n_classes == 1 else y, w.to(self.device)
                )
                ...
                if accumulated_samples < 128:
                    ...
                    fp = self.fp(
                        ...
                        accumulated_labels if self.n_classes == 1 else y,
                    )
                    self.n_fp += fp
                    self.history["recall"].append(
                        self.recall(accumulated_predictions, accumulated_labels)
                        .detach()
                        .cpu()
                        .numpy()
                    )
            ...
            if (
                step_ndx in val_steps
                and step_ndx > 1
                and false_positive_val_data is not None
            ):
            ...
            if (
                step_ndx in val_steps
                and step_ndx > 1
                and positive_test_clips is not None
            ):
                tp = 0
                fn = 0
            ...
            if step_ndx in val_steps and step_ndx > 1 and X_val is not None:
                ...
                for val_step_ndx, data in enumerate(X_val):
                    with torch.no_grad():
                        ...
                        val_predictions = self.model(x_val)
                        val_recall = (
                            self.recall(val_predictions, y_val[..., None])
                            .detach()
                            .cpu()
                            .numpy()
                        )
            ...
            if step_ndx in val_steps and step_ndx > 1:
                if self.history["val_n_fp"][-1] <= np.percentile(
                    self.history["val_n_fp"], 50
                ) and self.history["val_recall"][-1] >= np.percentile(
                    self.history["val_recall"], 5
                ):
...
if __name__ == "__main__":
    ...
    background_paths = []
    if len(config["background_paths_duplication_rate"]) != len(
        config["background_paths"]
    ):
        config["background_paths_duplication_rate"] = [1] * len(
            config["background_paths"]
        )
    for background_path, duplication_rate in zip(
        config["background_paths"], config["background_paths_duplication_rate"]
    ):
        background_paths.extend(
            [i.path for i in os.scandir(background_path)] * duplication_rate
        )
    ...
    duration_in_samples = []
    for i in range(n):
        sr, dat = scipy.io.wavfile.read(
            positive_clips[np.random.randint(0, len(positive_clips))]
        )
        duration_in_samples.append(len(dat))

    config["total_length"] = (
        int(round(np.median(duration_in_samples) / 1000) * 1000) + 12000
    )  # add 750 ms to clip duration as buffer
    ...
    if args.augment_clips is True:
        if (
            ...
            positive_clips_train = [
                str(i) for i in Path(positive_train_output_dir).glob("*.wav")
            ] * config["augmentation_rounds"]
            positive_clips_train_generator = augment_clips(
                positive_clips_train,
            ...
            positive_clips_test = [
                str(i) for i in Path(positive_test_output_dir).glob("*.wav")
            ] * config["augmentation_rounds"]
            positive_clips_test_generator = augment_clips(
                positive_clips_test,
                total_length=config["total_length"],
                batch_size=config["augmentation_batch_size"],
                background_clip_paths=background_paths,
            ...
            negative_clips_train = [
                str(i) for i in Path(negative_train_output_dir).glob("*.wav")
            ] * config["augmentation_rounds"]
            negative_clips_train_generator = augment_clips(
                negative_clips_train,
                total_length=config["total_length"],
                batch_size=config["augmentation_batch_size"],
                background_clip_paths=background_paths,
            ...
            negative_clips_test = [
                str(i) for i in Path(negative_test_output_dir).glob("*.wav")
            ] * config["augmentation_rounds"]
            negative_clips_test_generator = augment_clips(
                negative_clips_test,
                total_length=config["total_length"],
                batch_size=config["augmentation_batch_size"],
                background_clip_paths=background_paths,
            ...
            compute_features_from_generator(
                positive_clips_train_generator,
            ...
            logging.warning(
                "Openwakeword features already exist, skipping data augmentation and feature generation"
            )
    ...
    if args.train_model is True:
        ...
        def f(x, n=input_shape[0]):
            ...
            if n > x.shape[1] or n < x.shape[1]:
                x = np.vstack(x)
                new_batch = np.array(
                    [x[i : i + n, :] for i in range(0, x.shape[0] - n, n)]
                )
            else:
                return x
            return new_batch
        ...
        for key in (
            ["positive"]
            + list(config["feature_data_files"].keys())
        ...
        config["feature_data_files"]["positive"] = os.path.join(
            feature_save_dir, "positive_features_train.npy"
        )
        config["feature_data_files"]["adversarial_negative"] = os.path.join(
            feature_save_dir, "negative_features_train.npy"
        )
